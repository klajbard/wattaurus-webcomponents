...
class LinkSocial extends LitElement {
  constructor() {
    super();
    this.url;
    this.type;
...
const render$1$2 = (result, container, options) => {
    ...
    // fragment that is reused since nested renders can occur synchronously.
    const renderContainer = firstScopeRender ? document.createDocumentFragment() : container;
    render$5(result, renderContainer, Object.assign({ templateFactory: shadyTemplateFactory$3(scopeName) }, options));
    // When performing first scope render,
    // (1) We've rendered into a fragment so that there's a chance to
    // `prepareTemplateStyles` before sub-elements hit the DOM
    // (which might cause them to render based on a common pattern of
 ...
 * @nocollapse
 */
LitElement$3.render = render$1$2;

class LinkIcon extends LitElement$3 {
  constructor() {
    super();
    this.href = '';
    this.title = '';
